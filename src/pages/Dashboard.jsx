  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-6 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs font-semibold uppercase tracking-widest px-2.5 py-1 rounded-full ${styles.text}`}>{label}</span>
      </div>
      <div>
        {loading ? (
          ...
          <p className="text-5xl font-bold text-gray-900 tabular-nums leading-none">{value ?? "—"}</p>
        )}
        {!loading && !error && sub && <p className="text-xs text-gray-400 mt-2 leading-relaxed">{sub}</p>}
      </div>
    </div>
  );
}
...
function ReservasTable({ data, loading, error }) {
  ...
  const columns = useMemo(() => [
    {
      ...
      accessorFn: (row) => row.nombre || "Anónimo",
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        return <span className="text-gray-600 tabular-nums">{val}</span>;
      },
    },
    {
      id: "cupos",
      header: "Personas",
      accessorFn: (row) => row.personas || 1,
      cell: (info) => (
  ...
  if (error) return <p className="text-sm text-rose-500 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm mt-5">Error al cargar la tabla de reservas.</p>;

  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden mt-5">
      <div className="p-6 border-b border-gray-50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-amber-50 text-amber-600 rounded-lg"><ClipboardList className="w-4 h-4" /></div>
          <div>
        ...
        <table className="w-full text-left border-collapse">
              ...
              <tr key={headerGroup.id} className="bg-gray-50/50 border-b border-gray-100">
                {headerGroup.headers.map((header) => (
                  <th key={header.id} className="p-4 text-xs font-semibold tracking-wider text-gray-500 uppercase select-none">
                    <div className={`flex items-center gap-1 ${header.column.getCanSort() ? "cursor-pointer hover:text-gray-700" : ""}`} onClick={header.column.getToggleSortingHandler()}>
          ...
          <tbody className="divide-y divide-gray-100">
            {table.getRowModel().rows.length === 0 ? (
              <tr><td colSpan={5} className="p-8 text-center text-sm text-gray-400">No hay registros de reservas en el sistema.</td></tr>
            ) : (
      ...
      <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/30">
        ...
        <div className="flex gap-2">
          <button onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className="p-1.5 rounded-lg border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className="p-1.5 rounded-lg border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      ...
      const [empRes, incRes, evtRes, resRes] = await Promise.all([
        fetch(`${API}/emprendimientos`).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch(`${API}/incidencias`).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch(`${API}/eventos`).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch(`${API}/reservas`, { headers }).then(r => r.ok ? r.json() : null).catch(() => null),
      ]);

      setDashboardData({
        emp: { data: empRes ?? [], loading: false, error: !empRes },
        inc: { data: incRes ?? [], loading: false, error: !incRes },
        evt: { data: evtRes ?? [], loading: false, error: !evtRes },
        res: { data: resRes ?? [], loading: false, error: !resRes },
      });
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-12 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        <div className="mb-10">
          <p className="text-xs font-semibold uppercase tracking-widest text-emerald-700 mb-1">Panel de administración</p>
          <h1 className="text-4xl font-bold text-gray-900 sm:text-5xl tracking-tight">Dashboard</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
          <StatCard Icon={Briefcase} label="Emprendimientos" value={emp.data.length} sub={`${[...new Set(emp.data.map((e) => e.categoria))].length} categorías registradas`} variant="green" loading={emp.loading} error={emp.error} />
          <StatCard Icon={AlertTriangle} label="Incidencias" value={inc.data.length} sub={`${pend} pendientes de atención`} variant="red" loading={inc.loading} error={inc.error} />
          <StatCard Icon={Calendar} label="Eventos" value={evt.data.length} sub="Eventos registrados en el parque" variant="sky" loading={evt.loading} error={evt.error} />
          <StatCard Icon={Ticket} label="Reservas" value={res.data.length} sub="Total de reservas recibidas" variant="amber" loading={res.loading} error={res.error} />
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
          ...
          {emp.loading ? (
            <div className="flex flex-col gap-2">{[1, 2, 3].map((n) => <div key={n} className="h-6 rounded bg-gray-100 animate-pulse" />)}</div>
          ) : emp.error ? (
            ...
            <div className="flex flex-col gap-3">
              {Object.entries(emp.data.reduce((acc, e) => { acc[e.categoria] = (acc[e.categoria] || 0) + 1; return acc; }, {}))
                .sort((a, b) => b[1] - a[1])
                .map(([cat, count]) => (
                  <div key={cat} className="flex items-center gap-3">
                    <span className="text-xs text-gray-500 w-32 truncate">{cat}</span>
                    <div className="flex-1 bg-gray-100 rounded-full h-2">
                      <div className="bg-emerald-500 h-2 rounded-full transition-all duration-700" style={{ width: `${(count / emp.data.length) * 100}%` }} />
                    </div>
